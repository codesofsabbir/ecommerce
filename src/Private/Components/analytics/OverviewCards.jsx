	...
	return (
		<div className='grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4 mb-8'>
			{overviewData.map((item, index) => (
				<motion.div
					key={item.name}
					className='dark:bg-gray-800 bg-white bg-opacity-50 backdrop-filter backdrop-blur-lg shadow-lg
          ...
          '
				...
				>
					<div className='flex items-center justify-between'>
						<div>
							<h3 className='text-sm font-medium dark:text-gray-400 text-[#1E1E1E]'>{item.name}</h3>
							<p className='mt-1 text-xl font-semibold dark:text-gray-100 text-[#1E1E1E]'>{item.value}</p>
						</div>
